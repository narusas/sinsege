...
public class PdfFilteredReader extends PdfFileReader {
	...
	final public byte[] decodeFilters(byte[] data, String filter_list,
			Map objData, int width, int height, boolean useNewCCITT,
		...
		if (filter_list.length() > 0) {
			...
			while (filters_to_decode.hasMoreTokens()) {
				...
				boolean isDCT = (filter_type.indexOf("/DCTDecode") != -1)
						|| (filter_type.indexOf("/JPXDecode") != -1);
				...
				if (filter_type.startsWith("/")) {
					...
					// apply decode
					if ((filter_type.indexOf("/FlateDecode") != -1)
							| (filter_type.indexOf("/Fl") != -1)) {
						data = flateDecode(data, decodeParams, cacheName);

					} else if ((filter_type.indexOf("/ASCII85Decode") != -1)
							| (filter_type.indexOf("/A85") != -1)) {
						...
						if (checkJAI == false) {
							...
							checkJAI = true;
							// see if jai present
							java.io.InputStream in = getClass()
									.getClassLoader().getResourceAsStream(
											"javax/media/jai/RenderedOp.class");
							if (in == null) {
						...
						if ((useNewCCITT) && (hasJAI) && (data != null)) {
							TiffDecoder decode = new TiffDecoder(width, height,
									decodeParams, data);
						...
						if ((newData == null))
							data = ccittDecode(data, decodeParams, width,
									height);
						else
							data = newData;
	...
	private void setupCachedObjectForDecoding(byte[] data, String cacheName,
			final boolean debug, String filter_type) throws IOException,
		...
		if ((data != null) && (debugCaching)) {
			...
			if (debug)
				System.out.println("Cached stream " + filter_type
	 ...
	 * they are identical
	 */
	private byte[] runJBIGDecode(byte[] data,BufferedInputStream bis,
			BufferedOutputStream streamCache) throws Exception {
		
		//just copy of runlength at moment for structure
		ByteArrayOutputStream bos=null;
	...
	*
	 ...
	 */
	final private byte[] ccittDecode(byte[] data, Map values, int width,
	 ...
	 */
	final private byte[] ascii85DecodeOLD(byte[] data) {
		int special_cases = 0;
		String line = "";
	...
	final private byte[] ascii85DecodeNEW(byte[] valuesRead) {
		...
		// buffer to hold data
		byte[] temp_data = new byte[data_size - returns + 1
				+ (special_cases * 3)];
		int ii = 0;
		...
		for (int i = 0; i < data_size; i++) {
			...
			if (next == 122) {
				...
				// System.out.println("value="+value+" "+list);
				// and write out 4 bytes
				for (int i3 = 0; i3 < 4; i3++) {
					temp_data[output_pointer] = (byte) ((value / hex_indices[i3]) & 255);
					output_pointer++;
	 ...
	 */
	private void applyPredictorFunction(int mainPred, Map params, BufferedInputStream bis, 
			OutputStream bos) throws Exception {
