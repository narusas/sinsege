...
public class Filter {
	...
	static String[] DELETE_FILTER = { "대지권", "공담경정", "공담변경", "사항등기", "공동담보경정", "공동담보변경", "소멸", "추가", "질권",
			"부채권가압류", "담보", "부채권압류", "근저당권일부이전", "종결결정", "신탁등기경정", "신탁등기변경", "소유권\\d+분의\\d+등기", "근저당권지분일부이전",
			"등기명의인표시경정", "등기명의인표시변경", "일부변경", "신탁", "신탁등기", "신탁등기변경", "전세권공동목적변경", "근저당권\\d+지분일부이전", "사항$",
			"특약", "파산", "화의", "인가", "분할개시", "절차개시", "회사정리절차개시결정", "담보회복", "전사", "폐지", "지역권", "전세권저당권설정",
			"보전처리" };
	...
	public static void sortByDate(List<등기부등본_사항> items) {
		java.util.Collections.sort(items, new Comparator<등기부등본_사항>() {
			public int compare(등기부등본_사항 s1, 등기부등본_사항 s2) {
				return s1.getAcceptDateLong() == s2.getAcceptDateLong() ? 0 : s1.getAcceptDateLong() > s2
						.getAcceptDateLong() ? 1 : -1;
			}
		});
	}
