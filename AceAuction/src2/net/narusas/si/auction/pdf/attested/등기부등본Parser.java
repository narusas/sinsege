...
public class 등기부등본Parser {
	...
	private void collectColumnsBasic(List<TextPosition> list) {
		...
		while (buf1.size() != 0) {
			...
			if (isInContentsHeight(tp1) == false){
				continue;
			}
			if (tp1.getX2() >= 80 && tp1.getX2() < 130) {
				res1.add(tp1);
			} else if (tp1.getX2() >= 130 && tp1.getX2() < 280) {
				res2.add(tp1);
			} else if (tp1.getX2() >= 280 && tp1.getX2() < 370) {
				res3.add(tp1);
			} else if (tp1.getX2() >= 370 && tp1.getX2() < 500) {
				res4.add(tp1);
			} else if (tp1.getX2() >= 500) {
				res5.add(tp1);
			}
		}
	}
