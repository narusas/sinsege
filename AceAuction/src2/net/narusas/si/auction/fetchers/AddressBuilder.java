...
public class AddressBuilder {
	...
	public static class 통합주소 {
		...
		public String toString() {
			return "통합주소 [시도=" + 시도 + ", 시군구=" + 시군구 + ", 시군구그룹=" + 시군구그룹 + ", 읍면=" + 읍면 + ", 동리=" + 동리 + ", 번지=" + 번지 + ", 번지이하=" + 번지이하
					+ ", 도로명=" + 도로명 + ", 도로명그룹=" + 도로명그룹 + ", 건물주번=" + 건물주번 + ", 건물부번=" + 건물부번 + "]";
		}
	...
	private void parse도로명그룹(통합주소 _주소) {
		 String road = _주소.도로명;
		 _주소.도로명그룹 = roadGroup(road);
		 
	}
	
	String roadGroup(String road) {
		if (road == null){
			return null;
		}
		if (!road.matches(".+\\d+.+")){
		...
		if (road.matches(".+\\d+[번]?길$")){
			String[] tokens= road.split("\\d+[번]?길");
		...
		if (road.matches(".+\\d+거리")){
			String[] tokens= road.split("\\d+거리");
			String group = tokens[0];
			return group+"거리";
		}
		return null;
	...
	private 통합주소 parseAddress(String address) {
		...
		}
		{
			...
			if (_주소.시군구 != null && _주소.시군구.trim().contains(" ")) {
				_주소.시군구그룹 = _주소.시군구.substring(0, _주소.시군구.indexOf(" "));
			}
		}
	...
	private boolean is시도(String 시도) {
		return 시도.contains("서울") || 시도.contains("경기") || 시도.contains("경상") || 시도.contains("전라") || 시도.contains("충청") || 시도.contains("강원")
				|| 시도.contains("제주")

				|| 시도.contains("광주") || 시도.contains("대구") || 시도.contains("대전") || 시도.contains("부산") || 시도.contains("울산")
				|| 시도.contains("인천") || 시도.contains("세종");
	}
}
