...
public class StandardFonts {
	...
	public final static int CIDTYPE0=4;
	public final static int CIDTYPE2=5;
...
"Wingdings" };
		...
		// loop to read widths from default fonts
		{
			...
			while (true) { //read the lines and extract width info
				...
				if (line.startsWith("FontBBox")) {
					...
					StringTokenizer values = new StringTokenizer(line);
					values.nextToken();
					b_x1 = Integer.parseInt(values.nextToken());
					b_y1 = Integer.parseInt(values.nextToken());
					b_x2 = Integer.parseInt(values.nextToken());
					b_y2 = Integer.parseInt(values.nextToken());
	...
	private static void loadAdobeMap() {
		...
		if(adobeMap==null){
			try {
				...
				if (input_stream == null)
					LogWriter.writeLog(
					"Unable to open sglyphlist.cfg from jar");
				...
				while (true) {
					...
					if((!line.startsWith("#"))&&(line.indexOf(";")!=-1)){
						...
						/**
						//<end-13>
						unicode_name_mapping_table.put(key,""+((char)opVal));
						/**/
					}
				}
			} catch (Exception e) {
    ...
    public static String[] readPostscriptNames(int type, String subFont) throws Exception {
        ...
        /**
         * read font and convert to byte stream
         */
