...
private boolean processFlex(GlyphFactory factory, boolean debug, int lastKey, boolean isFlex, int routine) {
	...
	/** Utility method used during processing of type1C files */
		final public int getNumber(byte[] content, int pos,double[] values,int valuePointer,boolean debug) {
			...
			if ((b0 < 28) | (b0 == 31)) { //error!
				System.err.println("!!!!Incorrect type1C operand");
												  ...
												  // +32767
				x = (content[pos + 1] << 8) + (content[pos + 2] & 0xff);
				pos += 3;
			}else if(b0==255){

				if(is1C){
					int top=((content[pos + 1] & 0xFF)<<8)+(content[pos + 2] & 0xFF);
					if(top>32768)
						top=65536-top;
					double numb =top;
					double dec =((content[pos + 3] & 0xFF) << 8)+ (content[pos + 4] & 0xFF);
					x=numb+(dec/65536);
					if(content[pos + 1]<0){
						if(debug)
					...
					if(debug){
						...
						for(int j=0;j<5;j++){
							System.out.println(j+" "+content[pos+j]+ " "+(content[pos+j] & 0xff)+" "+(content[pos+j] & 0x7f));
						}
					}
				}else{
				//x=((content[pos + 1]& 127) << 24) + (content[pos + 2]<<16)+(content[pos + 3] << 8) + content[pos + 4];
				x =
					((content[pos + 1] & 0xFF) << 24)
						+ ((content[pos + 2] & 0xFF) << 16)
						+ ((content[pos + 3] & 0xFF) << 8)
						+ (content[pos + 4] & 0xFF);
				...
				x = ((b0 - 247) << 8) + (content[pos + 1] & 0xff) + 108;
				pos += 2;
			} else { //-1131 -108
				x = - ((b0 - 251) << 8) - (content[pos + 1] & 0xff) - 108;
				pos += 2;
			}
...
/*
	...
	protected void decodeGlyph(String embKey,GlyphFactory factory,String glyph,int rawInt, String displayValue, float currentWidth,boolean isRecursive) {
		 ...
		 */
		if((isCID)){
			glyphStream=(byte[]) charStrings.get(""+rawInt);
		}else{
			if(glyph==null)
				...
				if (nextVal >31 || nextVal==28) {  //if its a number get it and update pointer p
					...
					if (key ==12) { //handle escaped keys (ie 2 byte ops)
						...
						if(key==0){ //reserved
							...
							if(debug)
								System.out.println(key+" -------------- "+lastVal+" "+allowAll);
							
							if(commandCount>0 && key==10 && (lastVal>=0)&&(lastVal<=2) && lastKey!=11){//last key stops spurious match in multiple sub-routines
								isFlex = processFlex(factory, debug, lastKey, isFlex, lastVal);
								operandReached=0; //move to first operator
...
//								factor in bias
						...
						if(debug && !isNumber){
							...
							transformedGlyph2.render(GraphicsState.STROKE,g2,debug, 1f);
