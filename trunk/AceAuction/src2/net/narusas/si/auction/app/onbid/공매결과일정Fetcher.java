...
public class 공매결과일정Fetcher {
	...
	// http://www.onbid.co.kr/frontup/portal/result/control/result/getKamcoAuctionList.do?pageSize=10&pageIndex=2&ORDERFIELD=BID_OPEN_DATE&FLAG=DESC
	public List<공매결과일정> fetch() throws HttpException, IOException {
		int pageNo = 1;
		PageFetcher f = new PageFetcher("http://www.onbid.co.kr.");
		String page = f.fetch(pageUrl(pageNo));
		Pattern totalPattern = Pattern.compile("javascript:paging\\((\\d+)\\)");
		Matcher pageMatcher = totalPattern.matcher(page);
		...
		do {
			...
			System.out.println(pageUrl(pageNo));
			page = f.fetch(pageUrl(pageNo));
		} while (pageNo++ <= total);
	...
	private String pageUrl(int pageNo) {
		return "/frontup/portal/result/control/result/getKamcoAuctionList.do?pageSize=10&pageIndex="
				+ pageNo + "&ORDERFIELD=BID_OPEN_DATE&FLAG=DESC";
	}
}
