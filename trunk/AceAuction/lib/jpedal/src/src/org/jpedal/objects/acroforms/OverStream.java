...
public class OverStream implements FormDecoder {
	...
	final protected static boolean debug = false;//print info to screen
	...
	/** used to turn on and off appearance streams for forms 
	* in code until ready for enabling "in production"
	*/
    public static boolean debugSpecific = false;
	...
	protected Rectangle createBoundsRectangle(String rect,FormObject formObject) {
        ...
        Rectangle bBox = new Rectangle(rectX1,rectY1,rectWidth,rectHeight);
...
//		4=cmyk
	...
	public void workOutCharachteristic(String annotationFlags,FormObject formObject) {
		...
		if(debug){
            ...
            boolean[] index = formObject.characteristic;
            if(index!=null){
...
//					System.out.println("rgb="+r+","+g+","+b+" rg     CHECK ="+currentAt.textColor);
	...
	public FormObject createAppearanceString(Map currentField, PdfObjectReader currentPdfFile) {
		return null;
	}
}
