...
public class StructuredContentHandler {
    ...
    private Map keys,values,currentDictionary,dictionaries;
    ...
    public void BDC(Map dictionary) {
        ...
        if(MCID!=null){
        	...
        	dictionaries.put(""+markedContentLevel,dict);
    	}else{ //try to use tags if no structure
    		dictionaries.put(""+markedContentLevel,dictionary);
    	}
    ...
    public void EMC() {
        ...
        **/
        ...
        if(buildDirectly){
			...
			if(debug)
				System.out.println(isBMC+" "+currentKey+" "+dict);
			...
			if(isBMC){
				if(currentKey!=null){
					...
					if(addCoordinates){
						root.setAttribute("x1",""+(int)x1);
						root.setAttribute("y1",""+(int)y1);
						root.setAttribute("x2",""+(int)x2);
						root.setAttribute("y2",""+(int)y2);
					}	
				...
				if(addCoordinates){
					tag.setAttribute("x1",""+(int)x1);
					tag.setAttribute("y1",""+(int)y1);
					tag.setAttribute("x2",""+(int)x2);
					tag.setAttribute("y2",""+(int)y2);
				}
            ...
            Map dict=(Map) (dictionaries.get(""+markedContentLevel));
            ...
            dictionaries.remove(""+markedContentLevel);
